...
class CartTest extends TestCase
{
    ...
    public function TotalBruttoPriceWithTwoProducts(): void
    {
        $product1 = $this->buildTestProduct(1, 1000, '23%');
        $product2 = $this->buildTestProduct(2, 600, '8%');
        ...
        $this->assertEquals(3108, $cart->getTotalPriceBrutto());
    }
    ...
    public function SinglePriceWithTax()
    {
        $product = $this->buildTestProduct(1, 1000, '5%');

        $cart = new Cart();

        $cart->addProduct($product, 2);

        $this->assertEquals(2100, $cart->getBruttoPriceSingleProduct($product));
    }
    ...
    {
        $cart = new Cart();
        $cart->addProduct($this->buildTestProduct(1, 15000, '23%'), 1);
        ...
        $this->assertEquals(['id' => 7, 'items' => [
            ['id' => 1, 'quantity' => 1, 'total_price' => 15000, 'total_price_brutto' => 18450,'tax' => 3450],
            ['id' => 2, 'quantity' => 2, 'total_price' => 20000, 'total_price_brutto' => 21000, 'tax' => 1000],
        ], 'total_price' => 35000, 'total_price_brutto' => 39450], $order->getDataForView());
    }
